...
import { CreateAppointmentData, UpdateAppointmentData, CalendarFilters } from '../../interfaces/calendar/calendarInterfaces';
...
export class CalendarService {
  ...
  static async getCalendarEventsByType() {
    throw new Error('Method not implemented');
  }
  ...
  static async createAppointment(data: CreateAppointmentData, createdBy: number) {
    ...
    try {
      ...
      if (data.participants && data.participants.length > 0) {
        const participantValues = data.participants.map((userId: number) =>
      ...
      if (data.guests && data.guests.length > 0) {
        const guestValues = data.guests.map((guest: any) =>
