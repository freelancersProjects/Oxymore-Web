...
import { updateBodyClass, shouldHideSidebar, shouldHideHeader, shouldHideProfileSidebar, isOxiaPage } from "./config/layoutConfig";
import Friends from "./pages/Friends/Friends";
import Messages from "./pages/Messages/Messages";

const LayoutManager: React.FC<{
  children: (layout: { hideSidebar: boolean; hideHeader: boolean; hideProfileSidebar: boolean; isOxia: boolean }) => React.ReactNode;
}> = ({ children }) => {
  ...
  return <>{children({ hideSidebar, hideHeader, hideProfileSidebar, isOxia })}</>;
};
            ...
            return (
              <div className={`oxm-layout${isSidebarCollapsed ? " sidebar-collapsed" : ""}${isOxia ? " oxia-mode" : ""}`}>
                ...
                <div className={`oxm-main${isLoginPage ? ' oxm-main--no-margin' : ''}`}>
                  {!hideHeader && <Header isSidebarCollapsed={isSidebarCollapsed} hideProfileSidebar={hideProfileSidebar} />}
                  <main className={isLoginPage ? 'oxm-main--no-margin' : isFullBackgroundPage ? 'oxm-main--full-background' : ''}>
                    <ScrollToTop />
                    <Routes>
